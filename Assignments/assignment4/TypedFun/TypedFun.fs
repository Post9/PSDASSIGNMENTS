...
(* A type is int, bool, function or list *)
...
(* Exercise 5.7: examples with lists *)

(* [1; 2] : int list *)
let exL1 = ConC(CstI 1, ConC(CstI 2, CstN TypI));;

(* let sum (xs : int list) : int = match xs with [] -> 0 | h :: t -> h + sum t
   in sum [1; 2; 3] *)
let exL2 = Letfun("sum", "xs", TypL TypI,
                  Match(Var "xs", CstI 0,
                        ("h", "t", Prim("+", Var "h", Call(Var "sum", Var "t")))),
                  TypI,
                  Call(Var "sum", ConC(CstI 1, ConC(CstI 2, ConC(CstI 3, CstN TypI)))));;

(* a function returning a list: let tl (xs : bool list) : bool list = ... *)
let exL3 = Letfun("tl", "xs", TypL TypB,
                  Match(Var "xs", CstN TypB, ("h", "t", Var "t")),
                  TypL TypB,
                  Call(Var "tl", ConC(CstB true, CstN TypB)));;

let typesL = List.map typeCheck [exL1; exL2; exL3];;

(* List type errors; should throw exception when run: *)

(* elements of different types: 1 :: true :: [] *)
let exLErr1 = ConC(CstI 1, ConC(CstB true, CstN TypB));;

(* matching on something that is not a list *)
let exLErr2 = Match(CstI 5, CstI 0, ("h", "t", Var "h"));;

(* branches of different types: [] gives int, h :: t gives bool *)
let exLErr3 = Match(CstN TypB, CstI 0, ("h", "t", Var "h"));;
